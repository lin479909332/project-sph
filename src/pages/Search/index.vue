      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li
              class="with-x"
              v-for="(attrValue, index) in this.searchParams.props"
              :key="index"
            >
              {{ attrValue.split(":")[1]
...
export default {
  ...
  methods: {
    ...
    removeKeyword() {
      ...
      // 通过全局事件总线 完成兄弟组件通信问题（删掉header）的关键字
      this.$bus.$emit("clear");
      // 查看是否存在query参数，跳转时要保留query参数
      if (this.$route.query) {
        this.$router.push({ name: "search", query: this.$route.query });
      }
    },
    trademarkInfo(trademark) {
      //格式化数据 "ID:品牌名称" 这里不要加空格
      this.searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`;
      //searchParams参数发生变化 重新发送请求
      this.getData();
    },
    removeTrademark() {
      // 清空tradeMark，这里不用undefined，避免split报错
    ...
    changeOrder(flag) {
      ...
      //当用户点击的是综合的时候
      if (flag == originFlag) {
        //综合标签不变，排序若是升序则改为降序，降序则改为升序
        newOrder = `${originFlag}:${originSort == "desc" ? "asc" : "desc"}`;
      } else {
        //当用户点击的不是综合时，修改当前活动标签为用户点击的标签，默认降序
        newOrder = `${flag}:${"desc"}`;
      }
      //保存order状态，重新发送请求
