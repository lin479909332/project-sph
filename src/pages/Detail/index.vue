        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="superSaleAttr in superSaleAttrList"
                :key="superSaleAttr.id"
              >
                <dt class="title">{{ superSaleAttr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: spuSaleAttrValue.isChecked == 1 }"
                  v-for="spuSaleAttrValue in superSaleAttr.spuSaleAttrValueList"
                  :key="spuSaleAttrValue.id"
                  @click="
                    changeActive(
                      spuSaleAttrValue,
                      superSaleAttr.spuSaleAttrValueList
                    )
                  "
                >
...
export default {
  ...
  mounted() {
    ...
    //↓自己mock来的数据
    this.$store.dispatch("getSuperSaleAttrList");
  },
  computed: {
    ...mapGetters([
      "categoryView",
  ...
  methods: {
    ...
    changeActive(spuSaleAttrValue, arr) {
      //这里不仅要拿到当前点击的元素（spuSaleAttrValue），还要拿到整个数组arr（即所有兄弟）
      //forEach遍历所有的元素，将所有的元素的isChecked修改为0
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      //再将自己点击的元素的isChecked修改为1
    ...
    changeSkuNum(event) {
      ...
      //如果是非法数据（NaN）和小于1的书，就强制变成1
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      } else {
        //如果是大于1的书，就向下取整，避免用户输入小数
    ...
    async addShopCart() {
      try {
        ...
        //路由跳转
        //简单的数据 例如skuNum可以直接通关query传递，复杂的数据，例如skuInfo还是通过session storage传递
